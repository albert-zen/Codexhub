...
Define how manager, worker, and reviewer agents cooperate.
...
- If two workers need the same file or behavior seam, manager serializes the work or narrows scopes.
- If a worker discovers scope expansion is required, it stops and reports the issue instead of proceeding.

## Review Policy Hook

Workers may run `review-loop` in `worker-single-pass` mode for non-trivial work. Managers run `review-loop` after each wave according to `review-policy.md`.
